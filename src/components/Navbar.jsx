    ...
    <>
      ...
      <nav className="bg-white text-slate-800 sticky top-0 z-50 border-b border-slate-200 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between">
          ...
          {/* LOGO & SUBTEXT */}
          <Link to="/" onClick={closeMobileMenu} className="flex items-center gap-2 sm:gap-3">
            <img src={logo} alt="zaman Traders" className="h-10 sm:h-12 w-auto" />
            <div className="flex flex-col">
              <span className="text-base sm:text-lg font-bold tracking-tight text-slate-900 leading-none">
                ZAMAN TRADERS
              </span>
              <span className="text-[10px] sm:text-xs text-emerald-700 font-semibold tracking-wider mt-1">
            ...
            >
              ...
              {dropdownOpen && (
                <div className="absolute top-full left-0 w-64 bg-white border border-slate-100 rounded-xl shadow-xl p-2 z-50">
                  <Link to="/products" className="block px-4 py-2 hover:bg-amber-100 rounded-lg text-slate-800 hover:text-emerald-900">All Products</Link>
          ...
          <button onClick={() => setIsOpen(!isOpen)} className="md:hidden text-slate-800 focus:outline-none p-1">
        ...
        {/* ================= MOBILE MENU ================= */}
        {isOpen && (
          <div className="md:hidden bg-white border-t border-slate-200 px-6 py-5 space-y-4 shadow-xl">
            <Link to="/" onClick={closeMobileMenu} className="block font-semibold text-slate-800 hover:text-emerald-700 transition">Home</Link>
            <Link to="/about" onClick={closeMobileMenu} className="block font-semibold text-slate-800 hover:text-emerald-700 transition">About Us</Link>
            
            {/* Mobile Dropdown */}
            <div>
              <button
                onClick={() => setDropdownOpen(!dropdownOpen)}
                className="w-full flex items-center justify-between font-semibold text-slate-800 hover:text-emerald-700 transition py-1"
              >
                <span>Products</span>
              ...
              {dropdownOpen && (
                <div className="mt-2 pl-4 space-y-2 border-l-2 border-amber-500">
                  <Link to="/products" onClick={closeMobileMenu} className="block text-sm font-medium text-slate-700 hover:text-emerald-700">All Products</Link>
                  <Link to="/products/jute-sacks" onClick={closeMobileMenu} className="block text-sm font-medium text-slate-700 hover:text-emerald-700">Jute Sacks & Bags</Link>
                  <Link to="/products/jute-yarn" onClick={closeMobileMenu} className="block text-sm font-medium text-slate-700 hover:text-emerald-700">Jute Yarn & Twine</Link>
                  <Link to="/products/geotextiles" onClick={closeMobileMenu} className="block text-sm font-medium text-slate-700 hover:text-emerald-700">Jute Geotextiles</Link>
                </div>
              )}
            </div>

            <Link to="/gallery" onClick={closeMobileMenu} className="block font-semibold text-slate-800 hover:text-emerald-700 transition">Gallery</Link>
            <Link to="/export" onClick={closeMobileMenu} className="block font-semibold text-slate-800 hover:text-emerald-700 transition">Export</Link>
            <Link to="/contact" onClick={closeMobileMenu} className="block font-semibold text-slate-800 hover:text-emerald-700 transition">Contact</Link>
            
            <div className="pt-2">
              <Link 
                to="/contact" 
                onClick={closeMobileMenu} 
                className="block w-full bg-emerald-700 text-white text-center py-3 rounded-xl font-bold shadow-md hover:bg-emerald-800 transition"
              >
                Contact Us
              </Link>
            </div>
